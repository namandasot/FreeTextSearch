...
def Developer(words,cityid):
    ...
    try:
        ...
        if "Developer_Name" in result_location:
          ...
          if response['entities'][i]['type']=='LOCATION' and not response['entities'][i]['name'].lower() in amenity_exclusion and len(response['entities'][i]['name'])>2:
                     ...
                     if not pos-3<0:
                         if (tagged_words[pos-2][0] in ['km','kms','kilometer','kilometers','m','meter','meters']) and (tagged_words[pos-3][1]=='CD'):
                                 radius.append(tagged_words[pos-3][0])  

                     if not pos+3>length:
                        if (tagged_words[pos+3][0] in ['km','kms','kilometer','kilometers','m','meter','meters']) and (tagged_words[pos+2][1]=='CD'):
                                 radius.append(tagged_words[pos+2][0])
